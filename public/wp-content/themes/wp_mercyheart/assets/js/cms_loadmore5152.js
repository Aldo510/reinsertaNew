jQuery(document).ready(function($) {
	...
	$.loadData = function(){
		...
		$.get(nextLink,function(data){
			...
			if(cs_more_obj.masonry=='masonry'){
				var newItem = $($(data).find('.cms-isotope-masonry-post').html());
				$('.cms-isotope-masonry-post').imagesLoaded(function(){
					$('.cms-isotope-masonry-post').isotope('insert', newItem);
					if(typeof prettyPhoto != 'underfined'){
	...
	if(pageNum <= max) {
		...
		$('.cs_pagination').append('<div class="cshero-placeholder-'+ pageNum +'"></div><p id="cshero-load-posts"><a data-loading="0" href="#" class="btn btn-default">'+text+'</a></p>');
